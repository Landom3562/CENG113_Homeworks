    ...
    for key in genes_dict_keys:
        ...
        if gene_counts[key] >= frag_len:
            ...
            for subset in range(subset_count):
                gene_part = ""
                for gene_index in range(subset*frag_len,subset * frag_len + frag_len): #Adding first 50(frag_len) gene one by one to new key.
                    gene_part += genes_of_key[gene_index]
                key_part = name + "|" + str(number1) + "-" + str(number1 + frag_len) #Creating the name of the key by using first number and last number as 50(frag_len) more of it.
...
def main():
    ...
    #Finding the values before clean procedure and after to find the removed 4mers and print the result.
